...
# UserService gRPC E2E Tests
# Tests all 11 UserService interfaces with proper authentication

echo "👥 UserService gRPC E2E Tests"
echo "================================"
...
# Test 9: ChangePassword
run_test "UserService.ChangePassword - Change current user password" \
    "grpcurl -plaintext \
    -import-path $PROTO_PATH \
    -proto $USER_PROTO \
    -d '{
        \"metadata\": {
            \"tenant_id\": \"$CURRENT_TENANT_ID\",
            \"user_id\": \"$CURRENT_USER_ID\",
            \"request_id\": \"test-change-password-$(date +%s)\"
        },
...
# Test 10: ResetPassword (use test user if created)
if [ -n "$TEST_USER_ID" ] && [ "$TEST_USER_ID" != "null" ]; then
...
# Test 11: DeleteUser (use test user if created, otherwise expect failure)
if [ -n "$TEST_USER_ID" ] && [ "$TEST_USER_ID" != "null" ]; then
