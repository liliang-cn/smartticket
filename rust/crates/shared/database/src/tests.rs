...
mod tests {
    ...
    async fn test_tenant_isolation_policies() {
        ...
        // Create test tenants
        sqlx::query("INSERT INTO tenants (id, name, domain, subscription_tier, max_users, data_residency_region) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(tenant1_id)
            .bind("Test Tenant 1")
            .bind("tenant1.test.com")
            .bind("Enterprise")
            .bind(1000i32)
            .bind("EU")
            .execute(db.pool())
        ...
        // Create admin user
        sqlx::query("INSERT INTO users (id, tenant_id, email, username, full_name, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7)")
            .bind(admin_user_id)
            .bind(tenant1_id)
            .bind("admin@test.com")
            .bind("admin")
            .bind("Admin User")
            .bind("$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj6hsCm1HLKS")
            .bind("TenantAdmin")
            .execute(db.pool())
            .await
        ...
        sqlx::query("INSERT INTO users (id, tenant_id, email, username, full_name, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7)")
            .bind(user1_tenant1_id)
            .bind(tenant1_id)
            .bind("user1@tenant1.test.com")
            .bind("user1_t1")
            .bind("User 1 Tenant 1")
            .bind("$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj6hsCm1HLKS")
            .bind("CustomerUser")
            .execute(db.pool())
            .await
        ...
        sqlx::query("INSERT INTO users (id, tenant_id, email, username, full_name, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7)")
            .bind(user1_tenant2_id)
            .bind(tenant2_id)
            .bind("user1@tenant2.test.com")
            .bind("user1_t2")
            .bind("User 1 Tenant 2")
            .bind("$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj6hsCm1HLKS")
            .bind("CustomerUser")
            .execute(db.pool())
            .await
    ...
    async fn test_ticket_number_generation() {
        ...
        sqlx::query("INSERT INTO users (id, tenant_id, email, username, full_name, password_hash, role) VALUES ($1, $2, $3, $4, $5, $6, $7)")
            .bind(user_id)
            .bind(tenant_id)
            .bind("test@testcompany.test.com")
            .bind("testuser")
            .bind("Test User")
            .bind("$2b$12$LQv3c1yqBWVHxkd0LHAkCOYz6TtxMQJqhN8/LewdBPj6hsCm1HLKS")
            .bind("CustomerUser")
        ...
        sqlx::query("INSERT INTO tickets (id, tenant_id, title, description, contact_id, created_by_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING ticket_number")
            .bind(ticket1_id)
            .bind(tenant_id)
            .bind("Test Ticket 1")
            .bind("First test ticket")
            .bind(user_id)
            .bind(user_id)
            .fetch_one(db.pool())
            .await
        ...
        let result = sqlx::query("INSERT INTO tickets (id, tenant_id, title, description, contact_id, created_by_id) VALUES ($1, $2, $3, $4, $5, $6) RETURNING ticket_number")
            .bind(ticket2_id)
            .bind(tenant_id)
            .bind("Test Ticket 2")
            .bind("Second test ticket")
            .bind(user_id)
            .bind(user_id)
            .fetch_one(db.pool())
            .await
        ...
        let ticket_number: String = result.get("ticket_number");
        assert!(ticket_number.starts_with("TES-2025-"));
        assert!(ticket_number.len() > 10);
    ...
    async fn test_sla_metrics_creation() {
        ...
        sqlx::query("INSERT INTO tickets (id, tenant_id, title, description, contact_id, created_by_id) VALUES ($1, $2, $3, $4, $5, $6)")
            .bind(ticket_id)
            .bind(tenant_id)
            .bind("Test Ticket with SLA")
            .bind("Test ticket for SLA metrics")
            .bind(user_id)
            .bind(user_id)
            .execute(db.pool())
            .await
    ...
    async fn test_audit_logging() {
        ...
        sqlx::query("INSERT INTO knowledge_articles (id, tenant_id, title, content, author_id) VALUES ($1, $2, $3, $4, $5)")
            .bind(article_id)
            .bind(tenant_id)
            .bind("Test Article for Audit")
            .bind("This is a test article to verify audit logging works correctly.")
            .bind(user_id)
            .execute(db.pool())
            .await
        ...
        sqlx::query("UPDATE knowledge_articles SET title = $1 WHERE id = $2")
            .bind("Updated Test Article for Audit")
            .bind(article_id)
            .execute(db.pool())
            .await
        ...
        let create_log = sqlx::query("SELECT action, new_values FROM audit_logs WHERE resource_id = $1 AND action = 'CREATE' ORDER BY created_at ASC LIMIT 1")
            .bind(article_id.to_string())
            .fetch_one(db.pool())
            .await
            .expect("Failed to fetch CREATE audit log");

        let action: String = create_log.get("action");
        let new_values: Option<String> = create_log.get("new_values");
