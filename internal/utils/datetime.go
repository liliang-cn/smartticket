...
func (dt DateTime) StartOfDay() DateTime {
	...
	return DateTime{Time: time.Date(year, month, day, 0, 0, 0, 0, dt.Location())}
}

// EndOfDay returns the end of the day.
func (dt DateTime) EndOfDay() DateTime {
	year, month, day := dt.Date()
	return DateTime{Time: time.Date(year, month, day, 23, 59, 59, 999999999, dt.Location())}
}
...
func (dt DateTime) StartOfWeek() DateTime {
	...
	start := dt.AddDate(0, 0, -(weekday - 1))
...
func (dt DateTime) StartOfMonth() DateTime {
	...
	return DateTime{Time: time.Date(year, month, 1, 0, 0, 0, 0, dt.Location())}
}

// EndOfMonth returns the end of the month.
func (dt DateTime) EndOfMonth() DateTime {
	year, month, _ := dt.Date()
	nextMonth := time.Date(year, month+1, 1, 0, 0, 0, 0, dt.Location())
...
func (dt DateTime) Add(years, months, days int) DateTime {
	return DateTime{Time: dt.AddDate(years, months, days)}
}
...
func (dt DateTime) Timezone(loc *time.Location) DateTime {
	return DateTime{Time: dt.In(loc)}
}
...
func (dr DateRange) Contains(dt DateTime) bool {
	return (dt.Equal(dr.Start.Time) || dt.After(dr.Start.Time)) &&
		(dt.Equal(dr.End.Time) || dt.Before(dr.End.Time))
}
